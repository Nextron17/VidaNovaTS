...
import { 
  ...
  FileText, Pencil, Plus, User, FileDown,
...
function ProfileContent() {
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
        ...
        <p className="text-slate-400 font-bold">Cargando historial clínico...</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 p-6 md:p-10 font-sans text-slate-800 pb-32">
      ...
      {/* 1. HEADER */}
      <div className="mb-8">
        <Link href="/navegacion/atencion/pacientes" className="inline-flex items-center gap-2 text-slate-400 hover:text-emerald-600 transition-colors font-bold text-sm mb-4">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
            ...
            <div className="flex flex-wrap gap-2 w-full lg:w-auto">
                <Link href={`/navegacion/atencion/pacientes/perfil/editar?id=${patient.id}`} className="px-4 py-3 bg-white border border-slate-200 text-slate-700 rounded-xl font-bold text-sm hover:bg-slate-50 transition-all flex gap-2 items-center">
                    <Pencil size={16}/> Editar Datos
                </Link>
                <Link href={`/navegacion/atencion/pacientes/perfil/pdf?id=${patient.id}`} className="px-4 py-3 bg-white border border-slate-200 text-slate-700 rounded-xl font-bold text-sm hover:bg-slate-50 transition-all flex gap-2 items-center">
                    <FileDown size={16}/> Descargar PDF
                </Link>
                
                {/* Botón Nuevo Seguimiento en COLOR ESMERALDA */}
                <Link href={`/navegacion/atencion/gestion/nuevo?patientId=${patient.id}`} className="px-6 py-3 bg-emerald-600 text-white rounded-xl font-bold text-sm hover:bg-emerald-700 shadow-lg shadow-emerald-200 transition-all flex gap-2 items-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <InfoCard icon={<ShieldCheck/>} color="blue" label="Aseguradora" value={patient.insurance || 'No registrada'} sub="Afiliación Validada"/>
      ...
      <div className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm">
        <div className="flex items-center gap-4 mb-8 border-b border-slate-100 pb-4">
            ...
            <h4 className="text-xl font-black text-slate-900">Historia de Gestión de Atención</h4>
            <span className="text-xs font-bold bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full">{patient.followups?.length || 0} Eventos</span>
        </div>

        {!patient.followups || patient.followups.length === 0 ? (
            <div className="text-center py-12 text-slate-400 font-medium">No se han registrado seguimientos aún.</div>
        ) : (
                ...
                {patient.followups.map((h: any) => {
                    ...
                    return (
                        <div key={h.id} className="relative group">
                            {/* Bolita de tiempo ESMERALDA si está realizado */}
                            ...
                            <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm hover:shadow-md hover:border-emerald-300 transition-all relative">
                                
                                <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4 border-b border-slate-50 pb-4 pr-10">
                                    <div>
                                        <h6 className="font-bold text-slate-900 text-base leading-tight">{h.serviceName || 'Gestión de Atención'}</h6>
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {h.cups && <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded uppercase">CUPS: {h.cups}</span>}
                                            {h.category && <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded uppercase">{h.category}</span>}
                                        </div>
                                    </div>
                                ...
                                <Link 
                                    href={`/navegacion/atencion/gestion?id=${h.id}`} 
                                    className="absolute top-6 right-6 p-2 text-slate-300 hover:text-emerald-600 hover:bg-emerald-50 rounded-full transition-all" 
                                    title="Ver Detalles"
                                >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-2 gap-x-4 mb-4">
                                    {meta.PROF && (
                                        <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
                                            <Stethoscope size={14} className="text-emerald-500"/> <span className="truncate">{meta.PROF}</span>
                                        </div>
                                    )}
                                    {meta.LUGAR && (
                                        <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
                                    ...
                                    {meta.RESP && (
                                        <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
                                            <UserCog size={14} className="text-purple-500"/> <span className="truncate">Atendido por: {meta.RESP}</span>
                                        </div>
                                    )}
                                    {meta.DX && (
                                ...
                                {meta.BARRERA && (
                                    <div className="mb-4 bg-red-50 border border-red-100 p-3 rounded-xl flex items-start gap-3">
                                        ...
                                        <div>
                                            <p className="text-xs font-black text-red-700 uppercase mb-0.5">Dificultad Detectada</p>
                                ...
                                {text && (
                                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                                        <p className="text-xs font-bold text-slate-400 uppercase mb-1 flex items-center gap-2">
                                            <FileText size={12}/> Nota de Evolución
                                        </p>
...
export default function Page() {
    return (
        <Suspense fallback={<div className="flex justify-center p-20"><Loader2 className="animate-spin text-emerald-600" size={40}/></div>}>
            <ProfileContent />
        </Suspense>
    );
}