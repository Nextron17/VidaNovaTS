...
export default function AlertasAtencionPage() {
  ...
  if (loading) {
      return (
        <div className="w-full min-h-screen bg-[#fcfdfe] flex flex-col items-center justify-center">
            ...
            <p className="text-slate-500 font-black tracking-widest uppercase text-xs animate-pulse">Sincronizando alertas operativas...</p>
        </div>
      );
  }
  ...
  if (totalAlertas === 0) {
      return (
        <div className="w-full min-h-screen bg-[#fcfdfe] flex flex-col items-center justify-center p-6 text-center">
            <div className="w-24 h-24 bg-blue-50 text-blue-600 rounded-[2rem] flex items-center justify-center mb-8 shadow-xl shadow-blue-100 ring-4 ring-white">
                <CheckCircle2 size={48}/>
            </div>
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">Operación al Día</h2>
            <p className="text-slate-500 max-w-md text-lg font-medium leading-relaxed">
                No tienes pacientes con inconsistencias de fechas ni retrasos críticos en tu lista de navegación.
            </p>
            {/* ✅ RUTA CORREGIDA: Apunta al panel de atención */}
            <Link href="/navegacion/atencion/pacientes" className="mt-10 px-10 py-4 bg-blue-600 text-white rounded-2xl font-black shadow-2xl shadow-blue-200 hover:bg-blue-700 transition-all transform active:scale-95 uppercase text-xs tracking-widest">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
        <div>
            <div className="flex items-center gap-4 mb-3">
                ...
                <h1 className="text-4xl font-black text-slate-900 tracking-tighter">Panel de Alertas</h1>
            </div>
            <p className="text-slate-500 font-medium text-lg max-w-xl leading-snug">
                Priorización de trámites demorados y corrección de expedientes.
            </p>
        </div>
        ...
        <div className="flex items-center gap-4">
            ...
            <div className="bg-blue-50 border border-blue-100 px-6 py-3 rounded-2xl shadow-sm flex items-center gap-3">
                ...
                <span className="text-sm font-black text-blue-900 uppercase tracking-widest">{totalAlertas} Tareas Críticas</span>
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="space-y-6">
              <div className="flex items-center gap-3 px-2">
                  ...
                  <div>
                    <h3 className="font-black text-slate-900 text-lg leading-none uppercase tracking-tight">Revisión de Expedientes</h3>
              ...
              <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden flex flex-col h-[650px]">
                  <div className="p-4 bg-rose-50/40 border-b border-rose-50 text-[10px] text-rose-700 font-black uppercase tracking-[0.1em] flex items-center gap-3 shrink-0">
                  ...
                  <div className="overflow-y-auto custom-scrollbar flex-grow p-6 space-y-4">
                      {inconsistencies.length === 0 ? (
                          ...
                          inconsistencies.map((item) => (
                            <div key={item.id} className="p-5 rounded-3xl border border-slate-100 hover:border-blue-200 transition-all bg-white group relative overflow-hidden shadow-sm hover:shadow-xl">
                                <div className="absolute top-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                     <History size={40} className="text-blue-50"/>
                                </div>
                                <div className="flex justify-between items-start mb-4 relative z-10">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <span className="text-[10px] font-bold text-blue-500 uppercase tracking-widest">{item.eps || 'No EPS'}</span>
                                        </div>
                                    </div>
                                    {/* ✅ RUTA CORREGIDA: Apunta al perfil de atención */}
                                    <Link 
                                      href={`/navegacion/atencion/pacientes/perfil?id=${item.id}`}
                                      className="text-[10px] font-black text-blue-600 bg-blue-50 px-4 py-2 rounded-xl hover:bg-blue-600 hover:text-white transition-all uppercase tracking-widest shrink-0"
                                    >
                                      Corregir
                                    </Link>
                                </div>
                                ...
                                <div className="text-xs text-slate-500 font-medium mb-5 pl-1 tracking-tight italic">
                                    "{item.proc || 'Trámite no especificado'}"
                                </div>
                                
                                <div className="grid grid-cols-2 gap-4 bg-slate-50/50 p-4 rounded-2xl border border-slate-50 relative z-10">
                                    <div className="space-y-1">
                                        ...
                                        <span className="font-black text-blue-600 text-xs">
                                            {item.fecha_sol ? new Date(item.fecha_sol).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' }) : 'N/A'}
                                        </span>
                                    </div>
                                    <div className="space-y-1 border-l border-slate-200 pl-4">
                                        ...
                                        <span className="font-black text-rose-600 text-xs">
                                            {item.fecha_cita ? new Date(item.fecha_cita).toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' }) : 'N/A'}
                                        </span>
                                    </div>
                                </div>
                            </div>
          ...
          <div className="space-y-6">
              <div className="flex items-center gap-3 px-2">
                  ...
                  <div>
                    <h3 className="font-black text-slate-900 text-lg leading-none uppercase tracking-tight">Retrasos Críticos</h3>
                    <p className="text-xs text-slate-400 font-bold mt-1 uppercase tracking-widest">Trámites con más de 30 días</p>
                  </div>
              </div>

              <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden flex flex-col h-[650px]">
                  <div className="p-4 bg-amber-50/40 border-b border-amber-50 text-[10px] text-amber-700 font-black uppercase tracking-[0.1em] flex items-center gap-3 shrink-0">
                      <History size={14}/>
                      Atención: Pacientes fuera del tiempo estándar de gestión
                  </div>
                  ...
                  <div className="overflow-y-auto custom-scrollbar flex-grow p-6 space-y-4">
                      {overdue.length === 0 ? (
                          <div className="h-full flex flex-col items-center justify-center text-slate-300 opacity-50">
                              <CheckCircle2 size={64} strokeWidth={1} className="mb-4"/>
                              <p className="font-black uppercase tracking-widest text-xs">Gestión al día</p>
                          </div>
                      ) : (
                          overdue.map((item) => (
                            <div key={item.id} className="p-5 rounded-3xl border border-slate-100 hover:border-amber-200 transition-all bg-white group shadow-sm hover:shadow-xl">
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{item.eps || 'No EPS'}</span>
                                        </div>
                                    </div>
                                    <div className="flex flex-col items-end shrink-0">
                                        <span className="bg-rose-100 text-rose-700 px-3 py-1 rounded-full text-[10px] font-black flex items-center gap-1 shadow-sm border border-rose-200 uppercase tracking-widest">
                                            <Clock size={10}/> {item.dias || 0} Días
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="p-4 bg-slate-50 rounded-2xl mb-4 border border-slate-100">
                                    ...
                                    <span className="text-xs font-bold text-slate-700 leading-tight block">{item.proc || 'Trámite no especificado'}</span>
                                </div>

                                {/* ✅ RUTA CORREGIDA: Apunta a la gestión operativa */}
                                <Link 
                                  href={`/navegacion/atencion/gestion?id=${item.id}`}
                                  className="w-full flex items-center justify-center gap-2 py-3 bg-white border border-slate-200 text-blue-600 rounded-xl text-xs font-black hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all shadow-sm"
                                >
