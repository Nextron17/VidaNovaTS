...
// Colores para las gráficas: Indigo, Blue, Slate, Rose
const COLORS_ESTADOS = ['#6366f1', '#3b82f6', '#10b981', '#f43f5e']; 
...
const WhatsAppActions = ({ tel, nombre }: { tel: string, nombre: string }) => {
  ...
  return (
    <div className="relative group inline-block">
      <a href={`https://wa.me/57${numbers[0]}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center w-6 h-6 bg-indigo-100 text-indigo-600 rounded-full hover:bg-indigo-200 transition-colors border border-indigo-200 shadow-sm">
...
export default function AdminDashboardPage() {
  ...
  const getRowStyle = (estado: string, dias: number, meta: number) => {
    if (estado === "REALIZADO") return "border-l-4 border-indigo-500 bg-indigo-50/5 hover:bg-indigo-50/10";
  ...
  const getBadgeStyle = (estado: string) => {
    switch (estado) {
        case 'REALIZADO': return 'bg-indigo-100 text-indigo-700 ring-1 ring-indigo-600/20';
    ...
    <div className="min-h-screen bg-[#fcfdfe] pb-24 font-sans text-slate-800 p-6 md:p-8">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-10">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-2">Gestión estratégica de <span className="text-indigo-600 font-extrabold">{stats.total || 0}</span> expedientes clínicos.</p>
        </div>
        <div className="flex flex-wrap gap-3">
          <Link href="/navegacion/admin/importar" className="group flex items-center gap-2 px-5 py-3 bg-white border border-slate-200 text-slate-600 hover:text-indigo-600 rounded-xl text-sm font-bold transition-all shadow-sm">
            <FileSpreadsheet size={18} className="text-indigo-500"/> Importar
          </Link>
          <button onClick={fetchData} className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white hover:bg-indigo-600 rounded-xl text-sm font-bold shadow-lg transition-all">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-end">
          <div className="md:col-span-12 lg:col-span-4">
            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1.5 block">Búsqueda Inteligente</label>
            <div className="relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-600 transition-colors" size={18} />
              <input type="text" placeholder="Paciente, Cédula, CUPS..." className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all" value={busqueda} onChange={(e) => { setBusqueda(e.target.value); setPage(1); }} />
            </div>
          </div>
          <div className="md:col-span-6 lg:col-span-2">
            <FilterSelect label="📅 Inicio" type="date" value={filtros.fechaIni} onChange={(e:any) => setFiltros({...filtros, fechaIni: e.target.value})} />
          </div>
          <div className="md:col-span-6 lg:col-span-2">
            <FilterSelect label="📅 Fin" type="date" value={filtros.fechaFin} onChange={(e:any) => setFiltros({...filtros, fechaFin: e.target.value})} />
          </div>
          <div className="md:col-span-6 lg:col-span-2">
          ...
          <div className="md:col-span-12 lg:col-span-9">
            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1.5 block flex justify-between">
                <span>Categorías de Filtrado</span>
                {filtros.cohorte.length > 0 && (
                    <button onClick={() => setFiltros({ ...filtros, cohorte: [] })} className="text-rose-500 hover:underline text-[10px]">Limpiar Todo</button>
                )}
            </label>
            <div className="flex flex-wrap gap-2 p-2.5 bg-slate-50 border border-slate-200 rounded-xl min-h-[52px] items-center">
              {filtros.cohorte.map((c) => (
                <span key={c} className="bg-indigo-600 text-white text-[10px] font-black px-3 py-1.5 rounded-lg flex items-center gap-2 shadow-sm animate-in zoom-in duration-200 uppercase">
                  {c.replace('=', '').replace('CAC ', '')}
                  <button onClick={() => setFiltros({ ...filtros, cohorte: filtros.cohorte.filter(item => item !== c) })} className="hover:bg-indigo-400 rounded-full p-0.5">
                    <X size={12} />
                  </button>
                </span>
              ))}
              <select className="flex-1 bg-transparent text-sm font-bold text-slate-600 outline-none cursor-pointer min-w-[200px] py-1" value="" onChange={(e) => {
                const val = e.target.value;
                if (val && !filtros.cohorte.includes(val)) setFiltros({ ...filtros, cohorte: [...filtros.cohorte, val] });
              }}>
                <option value="" disabled>+ Agregar Categoría...</option>
                <optgroup label="SERVICIOS">
                    {MODALIDADES.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                </optgroup>
                <optgroup label="DIAGNÓSTICOS CAC">
                    {CAC_GROUPS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                </optgroup>
              </select>
            </div>
          </div>
          ...
          <div className="md:col-span-12 lg:col-span-3 flex justify-end">
            <button type="button" onClick={fetchData} className="w-full lg:w-fit bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-3 rounded-xl text-sm font-black shadow-xl shadow-indigo-500/20 transition-all flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <KpiItem title="EXPEDIENTES" value={stats?.total || 0} sub="Base de datos global" color="blue" icon={<Layers size={22}/>} />
        <KpiItem title="ATENDIDOS" value={stats?.realizados || 0} sub="Cierre de navegación" color="green" icon={<CheckCircle2 size={22}/>} />
        <KpiItem title="POR GESTIONAR" value={stats?.pendientes || 0} sub="En espera de acción" color="orange" icon={<AlertCircle size={22}/>} />
        <KpiItem title="PROGRAMADOS" value={stats?.agendados || 0} sub="Citas confirmadas" color="purple" icon={<Calendar size={22}/>} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col">
            <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-indigo-500"></span> Distribución de Estados
            </h4>
        ...
        <div className="col-span-2 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm flex flex-col">
            <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-blue-600"></span> Frecuencia de Servicios
            </h4>
            <div className="h-64 w-full">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} margin={{top: 10, right: 10, left: 0, bottom: 0}}>
                        ...
                        <Bar dataKey="cantidad" fill="#6366f1" radius={[4, 4, 4, 4]} barSize={35} />
                    </BarChart>
                </ResponsiveContainer>
            </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="flex border-b border-slate-100 px-6 pt-4 gap-6 overflow-x-auto bg-slate-50/30">
            ...
            <TabButton label="Realizados" active={tabEstado === 'REALIZADO'} onClick={() => setTabEstado('REALIZADO')} count={stats.realizados} color="indigo" />
        ...
        <div className="overflow-x-auto min-h-[350px] relative">
            {loading && (
                <div className="absolute inset-0 bg-white/80 z-20 flex items-center justify-center flex-col gap-3 backdrop-blur-[2px]">
                    ...
                    <span className="text-slate-500 text-[10px] font-black tracking-widest uppercase">Sincronizando...</span>
                </div>
            )}
            ...
            <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50 text-slate-400 text-[10px] font-black uppercase tracking-[0.15em] border-b border-slate-200">
                    <tr>
                        <th className="px-6 py-4 w-12 text-center"><input type="checkbox" onChange={seleccionarTodo} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"/></th>
                        <th className="px-4 py-4">Información Paciente</th>
                        <th className="px-4 py-4">Trámite Clínico</th>
                        <th className="px-4 py-4 text-center">Tiempos</th>
                    ...
                    {patients.map((row) => (
                        <tr key={row.id} className={`group transition-all duration-150 ${getRowStyle(row.estado, row.dias, row.meta)}`}>
                            <td className="px-6 py-4 text-center">
                                <input type="checkbox" checked={seleccionados.includes(row.id)} onChange={() => toggleSeleccion(row.id)} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"/>
                            </td>
                            <td className="px-4 py-4">
                                <div className="font-black text-slate-800 text-sm group-hover:text-indigo-700 transition-colors uppercase">{row.paciente}</div>
                            ...
                            <td className="px-4 py-4">
                                <div className="flex flex-col gap-1">
                                    ...
                                    <span className="text-[9px] font-black text-indigo-600 uppercase bg-indigo-50 px-2 py-0.5 rounded-md w-fit">{row.cohorte}</span>
                                </div>
                            </td>
                            <td className="px-4 py-4 text-center">
                                <div className="flex flex-col items-center gap-1">
                                    ...
                                    <span className="text-[9px] text-slate-400 font-mono italic">Agenda: {row.fecha_cita || 'N/A'}</span>
                                </div>
                            </td>
                            <td className="px-4 py-4 text-center">
                            ...
                            <td className="px-4 py-4 text-center">
                                <div className="flex flex-col items-center">
                                    <span className={`text-[11px] font-black px-2 py-0.5 rounded ${row.dias > row.meta ? 'text-rose-600 bg-rose-50' : 'text-slate-600 bg-slate-100'}`}>{row.dias} días</span>
                                </div>
                            </td>
                            <td className="px-6 py-4 text-right">
                                <div className="flex justify-end gap-2">
                                    <Link href={`/navegacion/admin/pacientes/perfil?id=${row.id}`} className="p-2 bg-slate-50 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all border border-transparent hover:border-indigo-100"><Eye size={16}/></Link>
                                    <Link href={`/navegacion/admin/gestion?id=${row.id}`} className="p-2 bg-slate-50 rounded-lg text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-all border border-transparent hover:border-amber-100"><Edit size={16}/></Link>
                                </div>
                            </td>
                        </tr>
                    ))}
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-between items-center text-xs font-bold text-slate-400">
            ...
            <div className="flex gap-2">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="p-2 border border-slate-200 rounded-xl bg-white hover:bg-indigo-50 hover:text-indigo-600 disabled:opacity-50 transition-all"><ChevronLeft size={16}/></button>
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="p-2 border border-slate-200 rounded-xl bg-white hover:bg-indigo-50 hover:text-indigo-600 disabled:opacity-50 transition-all"><ChevronRight size={16}/></button>
            </div>
        </div>
      </div>
      ...
      {seleccionados.length > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 animate-in slide-in-from-bottom-4 duration-300">
            <button onClick={() => setShowMassModal(true)} className="bg-slate-900 text-white pl-6 pr-8 py-4 rounded-2xl shadow-2xl flex items-center gap-4 hover:bg-indigo-700 transition-all font-bold border border-white/10 group">
                <div className="bg-indigo-500 text-white w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs">{seleccionados.length}</div>
                <div className="flex flex-col items-start leading-none text-left">
                    <span className="text-[10px] text-indigo-300 font-bold uppercase tracking-widest">Ejecución Masiva</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95">
                <div className="bg-indigo-600 p-6 flex justify-between items-center text-white">
                    <h3 className="font-black text-lg flex items-center gap-2 tracking-tight uppercase"><Layers size={20}/> Procesamiento Múltiple</h3>
                    <button onClick={() => setShowMassModal(false)} className="text-indigo-200 hover:text-white transition-colors"><X size={24}/></button>
                </div>
                <div className="p-8">
                    <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-2xl mb-8 flex items-start gap-4">
                        <Info size={20} className="text-indigo-600 mt-0.5 shrink-0"/>
                        <span className="text-indigo-900 text-sm font-bold leading-tight">Estás aplicando cambios globales a {seleccionados.length} expedientes. Esta acción se registrará en la bitácora administrativa.</span>
                    </div>
                    <div className="space-y-6">
                        <div>
                            <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Asignar Nuevo Estado</label>
                            <select className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all" onChange={(e) => setMassUpdateData({...massUpdateData, status: e.target.value})}>
                                <option value="">-- Sin cambios de estado --</option>
                                <option value="EN_GESTION">🟡 En Gestión Operativa</option>
                                <option value="AGENDADO">🔵 Cita Programada</option>
                                <option value="REALIZADO">🟢 Gestión Exitosa</option>
                            </select>
                        </div>
                        <div>
                            <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Nota Técnica de Cierre</label>
                            <textarea rows={4} placeholder="Escribe el motivo de la actualización masiva..." className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium resize-none outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all" onChange={(e) => setMassUpdateData({...massUpdateData, observation: e.target.value})}></textarea>
                        </div>
                    </div>
                    <div className="flex gap-3 mt-10">
                        ...
                        <button onClick={handleMassUpdate} className="flex-1 py-4 rounded-2xl bg-indigo-600 text-white font-black shadow-xl shadow-indigo-600/30 flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all uppercase text-xs tracking-widest">
...
function FilterSelect({ label, options, type = "text", value, onChange }: any) {
    return (
        <div className="w-full">
            ...
            {type === 'date' ? (
                <input type="date" className="w-full py-3 px-3 text-sm font-bold bg-slate-50 border border-slate-200 rounded-xl text-slate-700 outline-none focus:bg-white focus:border-indigo-500 transition-all" value={value} onChange={onChange}/>
            ) : (
...
function KpiItem({ title, value, sub, color, icon }: any) {
    ...
    return (
        <div className="p-6 rounded-2xl bg-white border border-slate-200/60 shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 transition-all group">
...
function TabButton({ label, active, onClick, color, count }: any) {
    const activeClass = ({ orange: "text-amber-600 border-amber-500 bg-amber-50/30", blue: "text-blue-600 border-blue-500 bg-blue-50/30", indigo: "text-indigo-600 border-indigo-500 bg-indigo-50/30", gray: "text-slate-800 border-slate-800 bg-slate-100" } as any)[color];
    const badgeColor = ({ orange: "bg-amber-100 text-amber-700", blue: "bg-blue-100 text-blue-700", indigo: "bg-indigo-100 text-indigo-700", gray: "bg-slate-200 text-slate-700" } as any)[color];
    return (
